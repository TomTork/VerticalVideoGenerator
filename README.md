...
- левое видео масштабируется до 90x90, частично выходит за левый край и
  располагается на 70 пикселей ниже центра стыка;
...
Нижнее, левое и правое верхнее видео автоматически зацикливаются, если они
короче ускоренного основного видео. Итог длиннее 60 секунд автоматически
делится на `_part1` и `_part2` в найденной паузе около середины ролика по тем
же правилам, которые используются в `main.py`.
...
Произносимое слово увеличивается по word timestamps Whisper и сохраняет
крупный размер до смены строки. Сама строка при наличии паузы остается на
экране еще на 0.5 секунды дольше.
