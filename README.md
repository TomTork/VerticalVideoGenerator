...
Чтобы поменять верхнюю и нижнюю области местами, используется флаг:

```bash
--swap-main-bottom
```

При этом основное видео сохраняет размер `--main-ratio`, длительность,
ускорение, экспозицию, субтитры и основную аудиодорожку, но располагается
снизу. Второе видео занимает верхнюю область. Субтитры и левый оверлей
остаются привязаны к стыку областей.

Громкость основной дорожки задается в процентах:

```bash
--main-audio-volume-percent 300
```

`100` сохраняет базовый уровень, `300` усиливает дорожку в три раза.

Защита от цифрового перегруза настраивается отдельно:

```bash
--audio-peak-protection limiter
--audio-peak-protection softclip
--audio-peak-protection off
```

По умолчанию используется `limiter`. При больших значениях, например `800`,
входной коэффициент действительно равен `8.0`, но итоговые пики не могут
превысить цифровой потолок. Режим `off` сохраняет буквальное предварительное
усиление ценой возможного сильного клиппинга и искажений.
...
Минимальные отступы субтитров справа и слева задаются отдельным параметром:

```bash
--subtitle-side-margin 80
```

При разбиении строк учитывается их конечная ширина после увеличения всех слов
до значения `--subtitle-scale`.
